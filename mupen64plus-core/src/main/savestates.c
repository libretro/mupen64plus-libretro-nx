...
int savestates_load_libretro(const void *data, size_t size)
{
    ...
    /* Read the rest of the savestate */
    savestateSize = 16788244;
    savestateData = (unsigned char *)malloc(savestateSize);
    ...
    else // version >= 0x00010100  saves entire eventqueue plus 4-byte using_tlb flage
    {
        ...
        memcpy(savestateData, data + sizeof(header), savestateSize);
        memcpy(queue, data + sizeof(header) + savestateSize, sizeof(queue));
        memcpy(additionalData, data+ sizeof(header) + savestateSize + sizeof(queue), sizeof(additionalData));
    }

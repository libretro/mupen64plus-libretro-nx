...
static u_int isclean(void* addr)
{
  u_char *ptr=(u_char *)addr;
  ...
  //head
  if(*ptr==0x48) //movimm64 to ARG1_REG;
  {
    head=(struct ll_entry *)(*((uintptr_t *)(ptr+2)));
    ptr+=10;
  }
  else
    return 1;
  ...
  {
    verifier=(void*)((intptr_t)(ptr+1)+*((int *)(ptr+1))+4); // follow branch
    ptr+=5;
  }
  else
    return 1;

  if(verifier!=(void*)verify_code)
    return 1;
...
{
  // Careful about the code output here, is_clean needs to parse it.

...
void init_r4300(struct r4300_core* r4300, struct memory* mem, struct mi_controller* mi, struct rdram* rdram, const struct interrupt_handler* interrupt_handlers,
    unsigned int emumode, unsigned int count_per_op, unsigned int overclock_factor, int no_compiled_jump, int randomize_interrupt)
{
    ...
    init_cp0(&r4300->cp0, count_per_op, overclock_factor, new_dynarec_hot_state, interrupt_handlers);

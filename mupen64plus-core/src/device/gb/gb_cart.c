                              ...
                              size_t size)
 {
     ...
     else if (address >= 0xA000 && address < 0xC000)
     {
         ...
         case 0x0c:
             ...
             DebugMessage(M64MSG_VERBOSE, "MBC3 read from RTC: %04x <- %02x (bank %02x)", address, dst[0], gb_cart->ram_bank);
             break;
                               ...
                               size_t size)
 {
     ...
     else if (address >= 0xA000 && address < 0xC000)
     {
         ...
         if (!gb_cart->ram_enable) {
             ...
             DebugMessage(M64MSG_VERBOSE, "MBC3 write to RTC: %04x <- %02x (bank %02x)", address, value, gb_cart->ram_bank);
             write_mbc3_rtc_regs(&gb_cart->rtc, gb_cart->ram_bank - 0x08, value);
             latch_mbc3_rtc_regs(&gb_cart->rtc, 1);
             break;

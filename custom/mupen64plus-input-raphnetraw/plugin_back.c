...
static int max_controllers = MAX_CHANNELS;
static int g_portmaps_init[MAX_CHANNELS] = { 0 };
static struct portMap g_portmaps[MAX_CHANNELS] = { };
...
int pb_init(pb_debugFunc debugFn)
{
	...
	// assume every port is a raphnet device by default
	// this generates a standard { 0, 1, 2, 3 } portmap
	for (int i=0; i<MAX_CHANNELS; i++)
	{
		pb_updatePortMap(i, 1, 1);

#ifdef __LIBRETRO__
		// libretro will re-initialize
		g_portmaps_init[i] = 0;
#endif
	}
...
static void pb_freeAdapter(void * adapter)
{
	struct adapter *adap = (struct adapter *) adapter;

	if (adap->handle) {
		gcn64lib_suspendPolling(adap->handle, 0);
		gcn64_closeDevice(adap->handle);
	}

	memset(adap, 0, sizeof(struct adapter));
}
...
/**
 * \return The number of undefined portmaps.
 */
int pb_updatePortMap(int Control, int IsPlugged, int IsRaphnet)
{
	int i, last_chn, raw_chn;
	int map = 0;
	int undefined = 4;

	g_portmaps_init[Control] = 1;
	g_portmaps[Control].is_plugged = IsPlugged;
	g_portmaps[Control].is_raphnet = IsRaphnet;

	for (i=0; i<MAX_CHANNELS; i++)
	{
		last_chn = g_portmaps[i].raw_chn;
		raw_chn = (g_portmaps[i].is_raphnet == 1 ? map++ : DEVICE_NON_RAPHNET);

		g_channels[raw_chn].port = i;
		g_portmaps[i].raw_chn = raw_chn;

		if (last_chn != raw_chn)
			g_portmaps[i].last_chn = last_chn;
	}
...
int pb_scanControllers(void)
{
	...
	while (gcn64_listDevices(&adap->inf, lctx)) {
		...
		g_n_adapters++;
		if (g_n_adapters >= max_controllers)
			break;
	...
	// sort the adapters by channel count in descending order
	qsort(g_adapters, g_n_adapters, sizeof(struct adapter), pb_sortAdapters);
	...
	for (i=0; i<g_n_adapters; i++) {
		...
		// the adapter doesn't have any channels, or there's no more room
		if (adap->inf.caps.n_raw_channels <= 0 || g_n_channels >= max_controllers)
		{
			pb_freeAdapter(adap);
			continue;
		}

		// when adding an adapter, try to add the entire adapter, if possible
		if (g_n_channels + adap->inf.caps.n_raw_channels > max_controllers && i+1 < g_n_adapters)
		{
			pb_freeAdapter(adap);
			continue;
		}
		...
		for (j=0; j<adap->inf.caps.n_raw_channels; j++) {
			if (g_n_channels >= max_controllers) {
				continue;
			}
			g_channels[g_n_channels].adapter = adap;
			g_channels[g_n_channels].adap = i;
			g_channels[g_n_channels].chn = j;
			g_channels[g_n_channels].port = -1;

			for (k=0; k<MAX_CHANNELS; k++) {
				if (g_portmaps[k].raw_chn == g_n_channels)
					g_channels[g_n_channels].port = g_portmaps[k].raw_chn;
			}
...
int pb_sortAdapters(const void *a, const void *b)
{
	struct adapter *adapA = (struct adapter *) a;
	struct adapter *adapB = (struct adapter *) b;
	return (adapB->inf.caps.n_raw_channels - adapA->inf.caps.n_raw_channels);
}
...
int pb_controllerCommand(int Control, unsigned char *Command)
{
	...
	struct portMap portMap = g_portmaps[Control];
	...
	for (j=0; j<g_n_channels; j++)
	{
		...
		for (i=0; i<adap->n_ops; i++) {
			if (EXTENSION_RW_ONLY && (biops[i].tx_data[0] < 0x02))
				continue;
			op_was_extio[i] = 1;
			printf("Before blockIO: op %d, adap: %d, chn: %d, : tx: 0x%02x, rx: 0x%02x, data: ", i, j, biops[i].chn,
		...
		if (res == 0) {
			...
			for (i=0; i<adap->n_ops; i++) {
				...
				printf("After blockIO: op %d, adap: %d, chn: %d, : tx: 0x%02x, rx: 0x%02x, data: ", i, j, biops[i].chn,
						biops[i].tx_len, biops[i].rx_len);
...
int pb_readController(int Control, unsigned char *Command)
{
	...
	struct blockio_op *biops;
	struct portMap portMap = g_portmaps[Control];
	int Channel = (portMap.raw_chn == DEVICE_NON_RAPHNET ? portMap.last_chn : portMap.raw_chn);
	...
	if (!pb_commandIsValid(Channel, Command)) {
		return 0;
	}

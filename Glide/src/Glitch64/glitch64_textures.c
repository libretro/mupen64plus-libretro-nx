      ...
      int do_download)
{
   ...
   ptrTexImage2D(GL_TEXTURE_2D, 0, gltexfmt, width, height, 0, glpixfmt, glpackfmt, info->data);
   info->width = width;
   info->height = height;
...
grtexsource:
   ...
   if (!do_download)
      glBindTexture(GL_TEXTURE_2D, get_tex_id(startAddress+1));
   ptrTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, min_filter[index]);
   ptrTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, mag_filter[index]);
   ptrTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap_s[index]);
   ptrTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrap_t[index]);

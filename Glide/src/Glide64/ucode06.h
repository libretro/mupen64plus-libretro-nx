...
static void DrawImage (DRAWIMAGE *d)
{
   ...
   switch (d->imageSiz)
   {
      ...
      case 3:
         ...
         break;
      default:
         //FRDPDP("DrawImage. unknown image size: %d\n", d->imageSiz);
         return;
   }
...
static void uc6_obj_rectangle(uint32_t w0, uint32_t w1)
{
   ...
   if (d.imageAdrs > 4096)
   {
      //FRDPDP("tmem: %08lx is out of bounds! return\n", d.imageAdrs);
      return;
   }
...
#if 0
         //FRDPDP ("imagePtr: %08lx\n", d.imagePtr);
         //FRDPDP ("frameX: %f, frameW: %d, frameY: %f, frameH: %d\n", d.frameX, d.frameW, d.frameY, d.frameH);
         //FRDPDP ("imageX: %d, imageW: %d, imageY: %d, imageH: %d\n", d.imageX, d.imageW, d.imageY, d.imageH);
         //FRDPDP ("imageFmt: %d, imageSiz: %d, imagePal: %d, imageStride: %d\n", d.imageFmt, d.imageSiz, d.imagePal, stride);
         //FRDPDP ("scaleX: %f, scaleY: %f\n", d.scaleX, d.scaleY);
#endif
      }
      else
         return;
